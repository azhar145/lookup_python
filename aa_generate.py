...
def no_of_files():
    ...
    ##    print(p)
    print('\n\nnow ',k, ' files created in dir (/home/az2/Downloads/t1/)')
...
def delete_files():
    ...
    ##    print(p)
    print('Clean up - deleted files in dir (/home/az2/Downloads/t1/) ---> ',k, ' files')
...
def main ():
    ...
    m=0
    num_of_files_to_gen_count=int(input("enter no of files to create: ----->  "))+0
    print('no of files will be created in dir (/home/az2/Downloads/t1/) ----- >', num_of_files_to_gen_count,' will be created')
...
def hl7(m):
        ...
        insurance=['Blue Cross Blue Shield','Atena','Kaiser','Sharp',
                   'Medi-cal','Medicare','Cigna','UnitedHealth','Humana','Centene Corp',
                   'CVS','Molina Healthcare Inc.','California Physicians Service','Carefirst Inc',
                   'Metropolitan','Highmark Group','Anthem Inc','Health Net','UPMC Health System']
        t2=np.random.choice(insurance, size=1)
        ...
        g.write('||')
        sd = x.date_between(start_date="-40y", end_date="now")
        delta = datetime.timedelta(days=365*randint(18,90))
        bd = sd-delta
        g.write(str(bd))
        g.write('|M-||2106-3|')
        
        gf=str(x.address()).split()
        g1=" ".join(gf)
##        g.write(str(x.address()))
        g.write(g1)
